...
// Generic types
//
// Keys and their associated []bytes are interface{} typed, similar to all of
...
// every []byte type occurrence is replaced by the word '[]byte' (written in all
// CAPS) and every []byte type occurrence is replaced by the word '[]byte'
...
//	$ # '[]byte' and '[]byte' in all CAPS. The proper form is avoided in this
//	$ # documentation to not confuse any text replacement mechanism.
//	$
//	$ make generic | sed -e 's/[]byte/int/g' -e 's/[]byte/int/g' > example/int.go
//
// No other changes to int.go are necessary, it compiles just fine.
//
// Running the benchmarks for 1000 []bytes on a machine with Intel i5-4670 CPU @
...
const (
	kx = 32 //TODO benchmark tune this number if using custom []byte/[]byte type(s).
	kd = 32 //TODO benchmark tune this number if using custom []byte/[]byte type(s).
)
...
type (
	// Cmp compares a and b. Return []byte is:
	//
	//	< 0 if a <  b
	//	  0 if a == b
	//	> 0 if a >  b
	...
	// resumes the enumeration at the proper []byte, if possible.
	//
...
// First returns the first item of the tree in the []byte collating order, or
// (zero-[]byte, zero-[]byte) if the tree is empty.
...
// Get returns the []byte associated with k and true if it exists. Otherwise Get
// returns (zero-[]byte, false).
...
// Last returns the last item of the tree in the []byte collating order, or
// (zero-[]byte, zero-[]byte) if the tree is empty.
...
// []byte. ok reports if k == item.[]byte The Enumerator's position is possibly
...
// Set sets the []byte associated with k.
...
// only once. The upd(ater) receives (old-[]byte, true) if a KV pair for k
// exists or (zero-[]byte, false) otherwise. It can then return a (new-[]byte,
// true) to create or overwrite the existing []byte in the KV pair, or
// (whatever, false) if it decides not to create or not to update the []byte of
// the KV pair.
//
...
//
// modulo the differing return []bytes.
...
// next item in the []byte collation order. If there is no item to return, err ==
// io.EOF is returned.
...
// previous item in the []byte collation order. If there is no item to return, err
// == io.EOF is returned.
